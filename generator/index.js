...
module.exports = (api, options, rootOptions) => {
  ...
  if (fs.existsSync(`src/store/${options.moduleName}`)) {
    console.warn(`\nStore module ${options.moduleName} already exists`)
    return
  }
  ...
  if (!fs.existsSync(rootStoreFile)) {
    api.render({
      [rootStoreFile]: `${templatesRoot}/index.js`
    })
  }

  // inject import declaration for new store module
  api.injectImports(rootStoreFile, `import ${options.moduleName} from './${options.moduleName}'`)

  api.postProcessFiles((files) => {
    const ast = recast.parse(files[rootStoreFile])
